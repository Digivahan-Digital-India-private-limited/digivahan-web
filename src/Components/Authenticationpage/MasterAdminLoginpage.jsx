...
const MasterAdminLoginpage = () => {
  ...
  const { MasterAdminSignInwithOtp } = useContext(MyContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result) {
        localStorage.setItem("master_login_phone", phone);
        toast.success("OTP sent successfully 💛");
        setTimeout(() => navigate("/page/admin/master/otp", { state: { phone } }), 300);
      }
    } catch {
        ...
        >
          ...
          <div className="grad-bg w-full md:w-1/2 p-8 flex flex-col justify-between relative overflow-hidden">
            ...
            <div className={`relative z-10 anim-fade-right ${mounted ? "" : "opacity-0"}`}
              ...
              <h2 className="text-white text-2xl font-bold mb-2 drop-shadow">Master Admin Control Panel</h2>
            ...
            <p className={`text-yellow-100 text-xs mt-8 relative z-10 anim-fade-right ${mounted ? "" : "opacity-0"}`}
              ...
              © {new Date().getFullYear()} DigiVahan. Master Authorised personnel only.
            </p>
          </div>
          ...
          <div className="w-full md:w-1/2 p-8 flex flex-col justify-center bg-white">
            ...
            <p className={`text-xs text-gray-400 mt-6 text-center anim-fade-up ${mounted ? "" : "opacity-0"}`}
              ...
              Restricted to master authorised DigiVahan administrators only.
            </p>
          </div>
