...
const BaseQueryCategoryPage = ({ categoryTitle, queryTypes = [] }) => {
  ...
  const fetchQueries = async () => {
    try {
      ...
      if (response.data?.success) {
        ...
        // Filter by the specified queryTypes
        const filtered = allQueries.filter(q => {
          if (queryTypes.length === 0) return true;
          // Handle General Information Queries special case:
          if (queryTypes.includes("General")) {
            return q.query_type === "General" || q.query_type === "Contact Form" || !q.query_type;
          }
          return queryTypes.includes(q.query_type);
        });
        setQueries(filtered);
      }
    } catch (error) {
      console.error(`Failed to fetch queries for ${categoryTitle}:`, error);
...
export default BaseQueryCategoryPage;
