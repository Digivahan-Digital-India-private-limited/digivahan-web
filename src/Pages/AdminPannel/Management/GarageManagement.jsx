...
import {
  ...
  Warehouse, Activity, Clock, Database, FileText,
} from "lucide-react";
...
function DeleteModal({ vehicle, onConfirm, onCancel, loading }) {
  const sourceLabels = {
    ...
    challan:     "Challan Cache",
    vehicleinfo: "RC Details Cache",
  };
  const deletedItems = {
    garage:      ["User's garage record", "RTO API logs", "Challan webhook records", "Challan cache (RTOChallanCache)", "RC details cache (VehicleInfoData)"],
    challan:     ["Challan cache (RTOChallanCache)", "RTO API logs", "Challan webhook records"],
    vehicleinfo: ["RC details cache (VehicleInfoData)", "RTO API logs", "Challan cache", "Challan webhook records"],
  };
  const items = deletedItems[vehicle?.source] || [];
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-gray-900">Full Delete — {sourceLabels[vehicle?.source]}</h2>
          </div>
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-5">
          <p className="text-sm text-red-700 font-semibold mb-3">
            Delete all cached data for <span className="font-mono bg-red-100 px-1 rounded">{vehicle?.vehicle_id}</span>?
          </p>
          <div className="flex flex-col gap-1 mb-3">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Will be deleted:</p>
            {items.map((item, i) => (
          ...
          <div className="flex flex-col gap-1.5 text-xs text-gray-600 border-t border-red-200 pt-3">
          ...
          <p className="text-xs text-red-500 mt-3 font-medium">
            ⚠️ After deletion, the next search for this vehicle will hit the 3rd party API fresh. This cannot be undone.
          </p>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} disabled={loading}
            ...
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" />Deleting...</> : <><Trash2 className="w-4 h-4" />Delete All Data</>}
          </button>
        </div>
      </div>
    </div>
...
function StatusBadge({ status }) {
  const map = {
    ...
    SUSPENDED: "bg-orange-100 text-orange-700", DELETED: "bg-gray-100 text-gray-500",
    PENDING_DELETION: "bg-yellow-100 text-yellow-700",
  };
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${map[status] || "bg-gray-100 text-gray-500"}`}>{status || "N/A"}</span>;
}
...
function SourceBadge({ source }) {
  ...
  if (source === "vehicleinfo")
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700"><Database className="w-3 h-3" />RC Details</span>;
  return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-violet-100 text-violet-700"><Activity className="w-3 h-3" />Challan Data</span>;
}
...
export default function GarageManagement() {
  ...
  const [pagination, setPagination]       = useState({
    total: 0, totalPages: 1, garageCount: 0, challanCount: 0, vehicleInfoCount: 0, allCount: 0,
  });
  ...
  const applySuggestion = (s) => {
    setSearchInput(s); setSearch(s); setPage(1); setShowSuggest(false); setSuggestions([]);
  };
  const handleSearchSubmit = (e) => { e.preventDefault(); setSearch(searchInput); setPage(1); setShowSuggest(false); };
  const clearSearch = () => { setSearchInput(""); setSearch(""); setPage(1); setSuggestions([]); setShowSuggest(false); };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      // For garage source: first remove from garage, then wipe all cache
      if (deleteTarget.source === "garage") {
        await fetch(`${BASE_URL}/api/v1/garage/admin/delete-vehicle`, {
          method: "DELETE",
          headers: getAuthHeaders(),
          body: JSON.stringify({ user_id: deleteTarget.userId, vehicle_number: deleteTarget.vehicle_id }),
        });
      }
      // Always: wipe all cache records (works for all 3 source types)
      const res  = await fetch(`${BASE_URL}/api/v1/garage/admin/delete-challan-record`, {
        method: "DELETE",
        headers: getAuthHeaders(),
        body: JSON.stringify({ vehicle_number: deleteTarget.vehicle_id }),
      });
      const data = await res.json();
      if (data.status || deleteTarget.source === "garage") {
        showToast(`✅ All data for ${deleteTarget.vehicle_id} deleted. Next search will hit 3rd party API fresh.`, "success");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20">
      {toast && (
        <div className={`fixed top-5 right-5 z-[100] px-5 py-3 rounded-xl shadow-xl text-sm font-medium text-white transition-all duration-300 max-w-sm ${toast.type === "success" ? "bg-emerald-600" : "bg-red-600"}`}>
          {toast.msg}
        </div>
      )}
      {deleteTarget && (
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">View &amp; manage all cached vehicle data</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          <form onSubmit={handleSearchSubmit}>
            <div className="relative" ref={searchRef}>
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                ...
                <button type="submit" className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700 transition">Search</button>
              </div>
              {showSuggest && suggestions.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                  <div className="px-3 py-2 text-xs font-semibold text-slate-400 border-b border-slate-100 bg-slate-50">Vehicle number suggestions</div>
                  {suggestions.map((s) => (
        ...
        <div className="flex items-center gap-2 mb-4 bg-white rounded-2xl border border-slate-200 shadow-sm p-2">
          {tabs.map((t) => (
            <button key={t.key} onClick={() => changeTab(t.key)}
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded-full ${tab === t.key ? "bg-white/20 text-white" : "bg-slate-100 text-slate-500"}`}>{t.count}</span>
            </button>
          ))}
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 mb-4 flex items-start gap-2">
          ...
          <p className="text-xs text-amber-700">
            <strong>Delete = Full Wipe:</strong> Deleting any row removes all related records from RTOApiLog, ChallanWebhook, RTOChallanCache, and VehicleInfoData. The next search for that vehicle will hit the 3rd party API fresh (no cache).
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="divide-y divide-slate-100">
              {vehicles.map((v, idx) => (
                <div key={`${v.userId}-${v.vehicle_id}-${idx}`}
                  ...
                  <div className="col-span-3 flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      v.source === "garage" ? "bg-gradient-to-br from-blue-500 to-indigo-600" :
                      v.source === "vehicleinfo" ? "bg-gradient-to-br from-emerald-500 to-teal-600" :
                  ...
                  <div className="col-span-2"><SourceBadge source={v.source} /></div>
                  ...
                  <div className="col-span-2"><StatusBadge status={v.accountStatus} /></div>
                  <div className="col-span-1 flex justify-center">
                    <button onClick={() => setDeleteTarget(v)}
                      className="w-8 h-8 flex items-center justify-center rounded-lg transition opacity-0 group-hover:opacity-100 duration-200 bg-red-50 hover:bg-red-100 text-red-400 hover:text-red-700"
